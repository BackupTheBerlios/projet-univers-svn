    ...
    namespace Test {
      ...
      void TestEngineControl::basicTest()
      {
        ...
        /// now we can test the control...
        {
          ...
          Oriented* throttle = model->getObject("throttle")->getTrait<Oriented>() ;
          ...
          Ogre::Quaternion full(sqrt(0.5),sqrt(0.5),0,0) ;
//          std::cout << full.getPitch().valueDegrees()/0.9 << std::endl ;
          throttle->setOrientation(full) ;
          engine_control->controlEngine() ;
//          std::cout << engine->getAppliedForce().Newton().z ;
