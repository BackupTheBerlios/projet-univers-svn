...
namespace ProjetUnivers {
  ...
  namespace Base {
    ...
    namespace Test {
      ...
      Entier TempComposition::nombreDObjectsDetruits = 0 ;



      ///////////////
      // Teste l'ajout d'un élément
      void TestEnsembleComposition::testadd() 
      {
      
        // on ajoute un élément ...
        element = new TempComposition(1) ;
        referenceElement = element ;
        this->ensembleTeste.add(element.Liberer()) ;

        // puis on vérifie qu'il est dedans
        CPPUNIT_ASSERT(this->ensembleTeste.Contient(referenceElement) == VRAI) ;

        CPPUNIT_ASSERT(TempComposition::nombreDObjectsDetruits == 0) ;
      ...
      void TestEnsembleComposition::testremove() 
      {
        ...
        CPPUNIT_ASSERT(TempComposition::nombreDObjectsDetruits == 1) ;        
      ...
      {
        this->ensembleTeste.add(new TempComposition(1)) ;
        this->ensembleTeste.add(new TempComposition(2)) ;
      ...
      {
        TempComposition::nombreDObjectsDetruits = 0 ;
