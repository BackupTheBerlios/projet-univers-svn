  ...
  namespace Model {
    ...
    namespace {
                      ...
                      const Base::Association<PlanDAssemblage>& _plan)
      {
        ...
        Base::FonctionAssociationObjectObject<PlanDeComposant,Composant> association ;
            ...
            ++planComposant)
        {
          ...
          _assemblage->addObject(composant) ;
          
          association.add(*planComposant, composant) ;
          
          GestionnaireObjects::add(composant.Liberer()) ;
            ...
            ++typeDeContrainte)
        {
         ...
         pointDAttache2 = 
          ...
          _assemblage->addContrainte(contrainte) ;
    ...
    Assemblage::Assemblage(const Nom& _nom, 
                           const Base::Association<PlanDAssemblage>& _plan)
    : Object(_nom), plan(_plan)
    {
      Construire(this,_plan) ;
    }


    void Assemblage::addObject(
      const Base::Association<Positionne>& _objet)
    {
      this->composants.add(_objet) ;
    }

    void Assemblage::addContrainte(
...
//                       Exception("contrainte ne portant pas sur un objet"));
      ...
      this->contraintes.add(_contrainte) ;
    }

    std::string Assemblage::AfficherReference() const
    {
      std::string resultat ;
    ...
    std::string Assemblage::AfficherDefinition() const
    {
      std::string resultat ;
